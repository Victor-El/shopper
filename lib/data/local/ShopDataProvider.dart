...
class ShopDataProvider {
  ...
  Future<ShopItemData> insertShopItem(ShopItemData shopItemData) async {
    shopItemData.id = await db.insert(SHOP_TABLE_NAME, shopItemData.toMap(),
  ...
  Future<ShopItemData> getShopItemData(int id) async {
    var resultList =
        await db.query(SHOP_TABLE_NAME, where: "$ID = ?", whereArgs: [id]);
  ...
  Future<List<Map<String, dynamic>>> getAll() async {
    var resultList = await db.query(SHOP_TABLE_NAME, orderBy: ID);
    return resultList;
  }

  Future<int> updateShopItemData(ShopItemData shopItemData) async {
    return await db.update(SHOP_TABLE_NAME, shopItemData.toMap(),
  ...
  Future<int> deleteShopItemData(ShopItemData shopItemData) async {
    return await db.delete(SHOP_TABLE_NAME, where: "$ID = ?", whereArgs: [shopItemData.id]);
  }

  Future<int> deleteShopItemDataWithId(int id) async {
    return await db.delete(SHOP_TABLE_NAME, where: "$ID = ?", whereArgs: [id]);
  }

  Future<int> deleteAll() async {
    return await db.delete(SHOP_TABLE_NAME, where: "1");
  }
